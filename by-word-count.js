...
//   var range = sheet.getRange("A1:A500");
  ...
  for (var i=1; i<values.length; i++) {
    if (values[i][0].toLowerCase().includes("example") && values[i][0].split(" ").length >= 6) {
      color = "#ff0000"; // kırmızı
    } else if (values[i][0].toLowerCase().includes("example") && values[i][0].split(" ").length == 5) {
      color = "#b7e1cd"; // yeşil
    } else if (values[i][0].toLowerCase().includes("example") && values[i][0].split(" ").length == 4) {
      color = "#fcd5b4"; // turuncu
    } else if (values[i][0].toLowerCase().includes("example") && values[i][0].split(" ").length == 3) {
      color = "#fff2cc"; // sarı
    } else {
      color = "#ffffff"; // beyaz
    }
